...
class Site extends SiteModule
{
	...
	$this->route('anjungan/poli2', 'getDisplayAntrianPoli2');
	...
	$poliklinik = str_replace(",", "','", $this->settings->get('anjungan.display_poli'));
    	$poli4d = implode(',', [2021091467, 2022012498, 2019092328, 2023021585]);
    	$query = $this->core->mysql()->pdo()->prepare("SELECT a.kd_dokter, a.kd_poli, b.nm_poli, c.nm_dokter, a.jam_mulai, a.jam_selesai FROM jadwal a, poliklinik b, dokter c WHERE a.kd_poli = b.kd_poli AND a.kd_dokter = c.kd_dokter AND a.hari_kerja = '$hari'  AND a.kd_poli IN ('$poliklinik') AND a.kd_dokter IN ($poli4d)");
    	$query->execute();
    	$rows = $query->fetchAll(\PDO::FETCH_ASSOC);;
        $result = [];
	$dokterdouble = [];
		...
		$current_time = date('H:i');
                if ($current_time >= $row['jam_mulai'] && $current_time <= $row['jam_selesai']) {
		$dokterdouble[] = $row['kd_dokter'];
                $row['dalam_pemeriksaan'] = $this->core->mysql('reg_periksa','pemeriksaan_ralan')
                  ->select('no_reg')
                  ->select('nm_pasien')
		  ->select('kesadaran')
		  ...
		  ->where('kd_poli', $row['kd_poli'])
                ...
                $row['selanjutnya'] = $this->core->mysql('reg_periksa')
                  ...
                  ->toArray();
		if(count($row['selanjutnya']) > 1) {
		  array_shift($row['selanjutnya']);
		  }
...
public function getDisplayAntrianPoli2()
  {
    $title = 'Display Antrian Poliklinik 2';
    $logo  = $this->settings->get('settings.logo');
    $display = $this->_resultDisplayAntrianPoli2();
    ...
    $content = $this->draw('display.antrian.poli2.html', [
      'title' => $title,
      'logo' => $logo,
      'powered' => 'Powered by <a href="https://mlite.id/">mLITE</a>',
      'username' => $username,
    ...
    public function _resultDisplayAntrianPoli2()
  {
    $date = date('Y-m-d');
    $tentukan_hari = date('D', strtotime(date('Y-m-d')));
    $day = array(
    ...
    $poliklinik = str_replace(",", "','", $this->settings->get('anjungan.display_poli'));
    $poli4d = implode(',', [2022052518, 2020021356, 2016061278, 2016042275, 202011355, 2022021494]);
    $query = $this->core->mysql()->pdo()->prepare("SELECT a.kd_dokter, a.kd_poli, b.nm_poli, c.nm_dokter, a.jam_mulai, a.jam_selesai FROM jadwal a, poliklinik b, dokter c WHERE a.kd_poli = b.kd_poli AND a.kd_dokter = c.kd_dokter AND a.hari_kerja = '$hari'  AND a.kd_poli IN ('$poliklinik') AND a.kd_dokter IN ($poli4d)");
    $query->execute();
    $rows = $query->fetchAll(\PDO::FETCH_ASSOC);;
    ...
    if (count($rows)) {
      foreach ($rows as $row) {
        $current_time = date('H:i');
        //if (!in_array($row['kd_dokter'], $dokterdouble)) {
        if ($current_time >= $row['jam_mulai'] && $current_time <= $row['jam_selesai']) {
          $dokterdouble[] = $row['kd_dokter'];
          $row['dalam_pemeriksaan'] = $this->core->mysql('reg_periksa', 'pemeriksaan_ralan')
            ->select('no_reg')
    ...
    public function getAjax()
    {
	  ...
	  echo '['.date('H:i:s').']';
		...
		$unique_poli = array();
		foreach ($query as $row) {
			if (!isset($unique_poli[$row['kd_poli']])) {
				$unique_poli[$row['kd_poli']] = $row;
				}
			}
                if(!empty($query)){
                    $data['status'] = 'ok';
                    $data['result'] = array_values($unique_poli);
                }else{
                    $data['status'] = 'err';
                    $data['result'] = '';
                }
